...
tgid="-1001595409571"  # ID чата для автобэкапа (Необходим доступ к сообщениям)
tg2id="-1001595409571" # ID чата диллерской группы для отправки ключей
admls="1809414249"	   # ID Админа
bot_api="5201546668:AAG6bbs2K_GAw9E0cgTJ5Uwm-mMIznWgrP0" # Токен бота 
...
View_User_info(){
	...
	echo -e " Порт\t    : ${Red}${port}${Font_default}"
	echo -e " Пароль\t    : ${Red}${password}${Font_default}"
	echo -e " Шифрование : ${Red}${method}${Font_default}"
	echo -e " Протокол   : ${Red}${protocol}${Font_default}"
	echo -e " Obfs\t    : ${Red}${obfs}${Font_default}"
	echo -e " Количество устройств : ${Red}${protocol_param}${Font_default}"
	echo -e " Общая скорость ключа : ${Red}${speed_limit_per_con} KB/S${Font_default}"
	echo -e " Скорость соединения у каждого пользователя : ${Red}${speed_limit_per_user} KB/S${Font_default}"
	echo -e " Запрещенные порты : ${Red}${forbidden_port} ${Font_default}"
	echo
	echo -e " Использованный трафик : Upload: ${Red}${u}${Font_default} + Download: ${Red}${d}${Font_default} = ${Red}${transfer_enable_Used_2}${Font_default}"
	echo -e " Осталось трафика : ${Red}${transfer_enable_Used} ${Font_default}"
	echo -e " Всего трафика : ${Red}${transfer_enable} ${Font_default}"
	echo -e "${Red}Ключ SS:${Font_default} ${ss_link}"
	echo -e "${Red}Ключ SSR:${Font_default} ${ssr_link}"
	echo && echo "==================================================="
}
# Создание юзера
Set_config_user(){
...
List_port_user(){
	...
	do
		...
		user_list_all=${user_list_all}"Пользователь: ${Red} "${user_username}"${Font_default} Порт: ${Yellow}"${user_port}"${Font_default} Трафик: ${Ocean}${transfer_enable_Used_2}${Font_default}\n"
	done
	Get_User_transfer_all
...
Del_port_user(){
	...
	do
		...
		if [[ ! -z ${del_user} ]]; then
			...
			if [[ -z "${match_del}" ]]; then
				echo -e "${Error} Удаление пользователя неуспешно ${Red}[Порт: ${Ocean}${del_user_port}]${Font_default} "
				break
			else
				Del_iptables
				Save_iptables
				curl -s -X POST https://api.telegram.org/bot"$bot_api"/sendMessage -d chat_id="$tg2id" -d parse_mode=Markdown -d text="Был удален ключ Shadowsocks %0A Порт: ${del_user_port} %0A Сервер: ${backup_serv_id}">> curl.tmp
				rm -r curl.tmp
				echo -e "${Info} Удаление пользователя успешно ${Red}[Порт: ${Ocean}${del_user_port}]${Font_default} "
				echo
...
${Purple}|${Font_default}${Red}Öndüriji:${Yellow} adaмzaт       ${Purple}            ${Font_default}${Purple}|${Font_default}
${Purple}|${Font_default}${Red}ICQ:${Yellow} @adamzat           ${Purple}            ${Font_default}${Purple}|${Font_default}
${Purple}|${Font_default}${Red}senesi: ${Yellow}[$(date +"%d-%m-%Y")]${Purple}                ${Font_default}${Purple}|${Font_default}
${Purple}|${Font_default}$(menu_status)${Purple}        ${Font_default}${Purple}|${Font_default}
${Purple}|${Font_default}${Red}Scriptyň wersiýasy: ${Yellow}v${sh_ver}${Font_default}            ${Purple}|${Font_default}
${Purple}|${Font_default}${Red}Onlaýn ulanyjylar: ${Yellow}${useronline}${Font_default}                ${Purple}|${Font_default}
${Purple}|————————————————————————————————————|${Font_default}
